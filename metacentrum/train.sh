...
# run training
python3 train.py --save $SCRATCHDIR/saved_images --data $DATADIR --epochs 2 > $HOMEDIR/training.out || { echo >&2 "Training ended up erroneously (with a code $?) !!"; exit 3; }
...
mv saved_images.zip $HOMEDIR || { echo >&2 "Result file(s) copying failed (with a code $?) !!"; exit 4; }
